...
$(function () {
    ...
    EarnCodeCollection = Backbone.Collection.extend({
        model : EarnCode,
        url : "TimeDetailWS.do?methodToCall=getEarnCodeJson&documentId=" + $('#documentId').val()
    });
    ...
    var TimeBlockView = Backbone.View.extend({
        ...
        events : {
            ...
            //"select #ui-datepicker-div" : "updateAssignmentsEarnCodesForDate",
                    ...
                    open : function () {
                        ...
                        if (!_.isUndefined(startDate) && !_.isUndefined(endDate)) {
                            ...
                            if ($("#selectedAssignment").is("input")) {
                                var dfd = $.Deferred();
                                dfd.done(self.fetchEarnCode(_.getSelectedAssignmentValue(), null, targetDay, timeBlock.get('earnCode')))
                        ...
                        var startSpanDate = Date.parse($("#startDate").val());
                        var endSpanDate = Date.parse($("#endDate").val());
                        if (startSpanDate != null && endSpanDate != null ) {
                            startSpanDate = startSpanDate.clearTime();
                            endSpanDate = endSpanDate.clearTime();
                    ...
                    buttons : {
                        "Add" : function () {
                            ...
                            if (!_.isEmpty($("#startDate").val()) && !_.isEmpty($("#endDate").val()) && !_.isEmpty($("#endTime").val())) {
                                ...
                                var startDateTime = Date.parse($('#startDate').val() + " " + $('#startTime').val());
...
//                                    && Date.compare(midnight, endDateTime) == 0
//                                    && Date.compare(startDateTime, endDateTime) != 0) {
//                                    $('#endDate').val(endDateTime.add(1).days().toString(CONSTANTS.TIME_FORMAT.DATE_FOR_OUTPUT));
//                                }
                                if (Date.compare(midnight, endDateTime) == 0
                                    && (Date.compare(endDate, startDate) >= 0)
                                    && $('.error-messages').text() == "") { //this is to avoid inceremnting with every click when an error is present
                                    $('#endDate').val(endDateTime.add(1).days().toString(CONSTANTS.TIME_FORMAT.DATE_FOR_OUTPUT));
                                }
                            }
                            ...
                            // if start and end dates are the same, the acrossdays should not be checked.
                            if (!_.isUndefined(startDate) && !_.isUndefined(endDate) && !_.isNull(startDate) && !_.isNull(endDate) && Date.compare(startDate,endDate) == 0) {
                                $('#acrossDays').val('n');
                                $('#acrossDays').removeAttr('checked');
                            ...
                            var canEditHoursOnly = _.isUndefined(timeBlock) || timeBlock == null ? false : timeBlock.get("canEditHoursOnly");
                            if (canEditAssignmentOnly && !canEditHoursOnly) {
        ...
        showOverTimeDialog : function (e) {
            ...
            if (!_.isUndefined(e.target.textContent) && e.target.textContent != "") {
                currentOvertimePref = e.target.textContent;
            }
            var dfd = $.Deferred();
            ...
            dfd.done(self.fetchOvertimeEarnCode(currentOvertimePref, timeBlock.get('startDate')))
                .done($("#overtimePref option[value='" + currentOvertimePref + "']").prop("selected", "selected"))
                .done($("#originalOvtEarnCode").val(currentOvertimePref))
...
//                                }
                        ...
                        $("#originalOvtEarnCode").appendTo("#time-detail");
        ...
        showTimeBlock : function (e) {
            ...
            // For more information : http://api.jquery.com/category/deferred-object/
        ...
        showLeaveBlock : function (e) {
            ...
            $("#spanningWeeks").prop("checked", spanningWeeks);
            dfd.done()
                .done(this.fetchEarnCode(leaveBlock.get("assignment"), isLeaveBlockReadOnly, startSpanDate, leaveBlock.get('earnCode')))
        ...
        applyRules : function (e) {
            ...
            if (e.get("canEditHoursOnly")) {
                // Make everything read only except the hours
                $("#timesheet-panel :input").prop("disabled", true);
                // hide the date picker
                $(".ui-datepicker-trigger").hide();
                $("#hours", $("#timesheet-panel")).prop("disabled", false);
                $("#selectedAssignment", $("#timesheet-panel")).prop("disabled", true);
            }
...
//            	$("#selectedEarnCode option[value='" + earnCode + "']").prop("selected", "selected");
        ...
        fetchOvertimeEarnCode : function (earnCode, startDate) {
            // Fetch earn codes based on the selected assignment
            // The fetch function is provided by backbone.js which also supports jQuery.ajax options.
        ...
        fetchEarnCodeAndLoadFields : function () {
            ...
            var dfd = $.Deferred();
            dfd.done(this.fetchEarnCode(_.getSelectedAssignmentValue(), null, $('#startDate').val(), null))
                .done(this.showFieldByEarnCodeType());
        },
        ...
        validateEarnCode : function () {
            ...
            if (_.contains(ids, "startTimeHourMinute") && _.contains(ids, "endTimeHourMinute")) {
                ...
                var hours = $('#hours');
                isValid = isValid && (this.checkEmptyField(hours, "Hour") && this.checkMinLength(hours, "Hour", 1) && this.checkZeroValue(hours, 'Hours cannot be zero') && this.checkNumeric(hours, /^-?\d*(\.\d{1,})?$/, 'Hours should be numeric'));
            }
            else if (_.contains(ids, "amount")) {
                var amount = $('#amount');
                isValid = isValid && (this.checkEmptyField(amount, "Amount") && this.checkMinLength(amount, "Amount", 1) && this.checkZeroValue(amount, 'Amount cannot be zero') && this.checkNumeric(amount, /^-?\d*(\.\d{1,})?$/, 'Amount should be numeric'));
            }
